...
export class ExpertLevelsService {
  ...
  async getExpertLevel(expertId: number): Promise<any> {
    try {
      // Expert 테이블에서 통계 정보 조회
      const expert = await this.prisma.expert.findUnique({
        where: { id: expertId },
        select: {
          id: true,
          // totalSessions: true, // Field exists in Expert model
          ratingAvg: true,
          reviewCount: true,
          repeatClients: true,
          // likeCount 필드가 없으면 기본값 사용
        },
      });
      ...
      const stats: ExpertStats = {
        totalSessions: 0, // Use default value since field not selected
        avgRating: expert.ratingAvg,
        reviewCount: expert.reviewCount,
        repeatClients: expert.repeatClients || 0,
        likeCount: 0, // 임시로 0 사용 (추후 필요시 추가)
      };
