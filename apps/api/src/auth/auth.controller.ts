...
export class AuthController {
  ...
  async refresh(
    ...
    try {
      const newTokens = await this.auth.refreshTokens(
        refreshToken,
        secrets,
        ttls
      )
  ...
  async googleCallback(
    ...
    const refreshPayload = JSON.parse(
      Buffer.from(refresh.split('.')[1], 'base64').toString()
    )
  ...
  async sendPhoneVerification(
    ...
    const result = await this.auth.sendPhoneVerification(
      dto.phoneNumber,
      userId
    )
  ...
  async verifyPhoneCode(
    ...
    const userId = req.user?.id // Optional: 로그인된 사용자
    const result = await this.auth.verifyPhoneCode(
      dto.phoneNumber,
      dto.code,
      userId
    )
  ...
  async exchangeTempToken(
    ...
    try {
      // tempToken 검증 및 사용자 정보 가져오기
      const result = await this.auth.verifyOAuthTempToken(tempToken)

      if (!result.valid || !result.user) {
        return fail(
          'INVALID_TOKEN',
          result.error || '유효하지 않은 토큰입니다.'
        )
      }
      ...
      const access = this.auth.signAccess(tempUser, secrets.access, ttls.access)
      const refresh = this.auth.signRefresh(
        tempUser,
        secrets.refresh,
        ttls.refresh
      )
      ...
      console.log(
        '[Exchange Temp Token] Setting cookies for user:',
        tempUser.id
      )
      res.cookie('access_token', access, this.cookieOpts(ttls.access))
      res.cookie('refresh_token', refresh, this.cookieOpts(ttls.refresh))
      ...
      console.error('[Exchange Temp Token] Error:', error)
      return fail(
        error.response?.error?.code || 'INTERNAL',
        error.response?.error?.message || '토큰 교환 중 오류가 발생했습니다.'
      )
    }
  }
  ...
  async completeOnboarding(
    ...
    try {
      const user = await this.auth.completeOAuthOnboarding(
        tempToken,
        agreedToTerms
      )
      ...
      return fail(
        error.response?.error?.code || 'INTERNAL',
        error.response?.error?.message || '온보딩 완료 중 오류가 발생했습니다.'
      )
    }
  }
}
